...
function creatTbodyData() {
    ...
    for (let i = 0; i < tableTbodyData.length; i++) {
        ...
        trHtml += `<tr>`
        // 头像
        trHtml += creatrFace(data.face)
        // 姓名
        trHtml += createPeopleName(data.name)
        // 性别
...
// 生成姓名
function createPeopleName(data) {
    let html = `<td>
...
// 生成头衔
function createGrade(data, number= '') {
    let num = number == '' ? 20 : number;
    let html =  `<td>
                <div class = "grade">
                    <span class="role-name">${data.name}</span>
                    <div class="grade-wrap icon-grade">
                        <div class="grade-hight icon-grade" style="width: ${data.level * num}%;"></div> 
                    </div> 
                </div>
                </td>`
    return html
